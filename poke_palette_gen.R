...
all_images <- unique(pkmn_df$file)

#for(i in 1:length(all_images)){
for(i in 1:20){
  
  chosen_file <- all_images[i]
  #chosen_file <- 'pokemon_icon_004_00_shiny.png'
  example <- filter(pkmn_df,file == chosen_file)
  ...
  ggsave(paste0("poke_palettes/",chosen_file)
         ,plot = output
